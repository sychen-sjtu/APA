...
#define BIAS_GYR 1
#define WS 1.1
#define IMU_INSTALL_BIAS 1

Eigen::Matrix3d ypr2R(const Vector3d &ypr)
    {
...
Vector3d imu_error = Vector3d(5.0 , 2.0, 1.0);
...
DataGenerator::DataGenerator()
{
    ...
    gps_cov = 0.01 * 0.01 * Matrix3d::Identity();
...
Vector3d DataGenerator::getPosition()
{
    ...
    Vector3d disturb = Vector3d(distribution(generator) * sqrt(gps_cov(0, 0)),
                                    distribution(generator) * sqrt(gps_cov(1, 1)),
                                    distribution(generator) * sqrt(gps_cov(2, 2)));
    return Vector3d(x, y, z) + disturb * GPS_NOISE;
}
...
Vector3d DataGenerator::getAngularVelocity()
{
    ...
    Vector3d bias = Vector3d(0.02, 0.03, 0.04);

    return ypr2R(imu_error)* (IMU_NOISE*disturb + BIAS_GYR*bias + Vector3d(skew(2, 1), -skew(2, 0), skew(1, 0)));
...
//     {
// #if BIAS_GYR
//         return ypr2Rotation(dimuInstallAngle) * Vector3d(skew(2, 1) + 0.02, -skew(2, 0) + 0.03, skew(1, 0) + 0.04);
// #endif
//         return ypr2Rotation(dimuInstallAngle) * Vector3d(skew(2, 1), -skew(2, 0), skew(1, 0));
//     }
...
Vector3d DataGenerator::getVelocity()
{
    ...
    return WS * getRotation().inverse() * Vector3d(dx, dy, dz);
}
...
Vector3d DataGenerator::getLinearAcceleration()
{
    ...
    Vector3d bias = Vector3d(0.01, 0.02, 0.03);

    return ypr2R(imu_error)*(IMU_NOISE*disturb + BIAS_ACC*bias + getRotation().inverse() * Vector3d(ddx, ddy, ddz + 9.805));
...
//     {
// #if BIAS_ACC
//         return ypr2Rotation(dimuInstallAngle) * (getRotation().inverse() * Vector3d(ddx, ddy, ddz + 9.805) + Vector3d(0.01, 0.02, 0.03));
// #endif
//         return ypr2Rotation(dimuInstallAngle) * (getRotation().inverse() * Vector3d(ddx, ddy, ddz + 9.805));
